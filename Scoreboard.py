...
class TradeBook:
  ...
  def calculate_pnl(self):
    ...
    self.averageLossSize = round(self.lossamount/self.losingtrades)
    self.averageWinSize = round(self.winamount/self.winningtrades)
    self.battingaverage = round(self.totalpnl/self.totalTrades)
    self.strikerate = round(self.averageWinSize/-self.averageLossSize)
...
class Scoreboard:
  def __init__(self):
    ...
    for row in ws.iter_rows(min_row=2):  # Skip the first row with the column titles
        ...
        stk.add_trade(trade)
        self.tradebook.add_stock(stk)
  def process_stats(self,tradebook):
    ...
    print(f'Largest Winner: {tradebook.largestWinner:,}')
    print(f'Largest Winner: {tradebook.largestLoser:,}')
    print(f"Average Win Per Trade : {tradebook.averageWinSize:,}")
    print(f"Average Loss Per Trade : {tradebook.averageLossSize:,}")
    print(f"Batting Average :{tradebook.battingaverage}")
    print(f'Strike Rate: {tradebook.strikerate:,}R')
    #tradebook.db.execute('insert into patterns(stock,pattern,pattern_date) values(?,?,?)',[symbol,name,dato])
...
if __name__ == '__main__':
  obj = Scoreboard()
  obj.process_stats(obj.tradebook)
